...
export class CadastrarComponent implements OnInit {
    ...
    async cadastrar() {
        ...
        formData.append('hexadecimal', this.dados['hexadecimal']);
        formData.append('tipo', this.dados['tipo']);
        formData.append('status', this.dados['status']);

        this.carregando = true;

        const retorno: any = await this.api.post('cores', formData)

        this.carregando = false;

        if (!retorno['erro']) {

            this.utils.exibirToast('sucesso', 'Cor cadastrada com sucesso!', 'Sucesso');
